...
const FLAGS: Record<string, string> = {
  'Argentina': '🇦🇷', 'France': '🇫🇷', 'England': '🏴󠁧󠁢󠁥󠁮󠁧󠁿', 'Brazil': '🇧🇷', 'Spain': '🇪🇸', 'Germany': '🇩🇪', 'Portugal': '🇵🇹', 'Netherlands': '🇳🇱',
  'Italy': '🇮🇹', 'Belgium': '🇧🇪', 'Uruguay': '🇺🇾', 'Croatia': '🇭🇷', 'Morocco': '🇲🇦', 'Colombia': '🇨🇴', 'USA': '🇺🇸', 'Canada': '🇨🇦',
  'Mexico': '🇲🇽', 'Switzerland': '🇨🇭', 'Japan': '🇯🇵', 'Senegal': '🇸🇳', 'South Korea': '🇰🇷', 'Poland': '🇵🇱', 'Saudi Arabia': '🇸🇦', 'Australia': '🇦🇺',
  'Ecuador': '🇪🇨', 'Sweden': '🇸🇪', 'Scotland': '🏴󠁧󠁢󠁳󠁣󠁴󠁿', 'Norway': '🇳🇴', 'Wales': '🏴󠁧󠁢󠁷󠁬󠁳󠁿', 'Denmark': '🇩🇰', 'Serbia': '🇷🇸', 'Ghana': '🇬🇭',
  'India': '🇮🇳', 'New Zealand': '🇳🇿', 'South Africa': '🇿🇦', 'Pakistan': '🇵🇰', 'Sri Lanka': '🇱🇰', 'West Indies': '🌴', 'Bangladesh': '🇧🇩', 'Afghanistan': '🇦🇫'
};
...
export default function AuctionRoom({ roomId, userId, teamId, userName, onLeave }: AuctionRoomProps) {
  ...
  const handleDownloadFullReport = () => {
    if (!roomState) return;
    const rows = [['Team Name', 'Owner', 'Total Budget', 'Spent', 'Remaining', 'Player Name', 'Role', 'Country', 'Base Price', 'Sold Price']];
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(roomRef, (snapshot) => {
      if (!active) return;
      if (snapshot.exists()) { setRoomState(snapshot.val()); setNotFound(false); } 
      else { setNotFound(true); }
    });
    return () => { active = false; off(roomRef, 'value', unsubscribe); };
  }, [roomId]);
  ...
  useEffect(() => {
    ...
    if (roomState.phase === 'bidding' && (lastPhaseRef.current === 'lobby' || lastPhaseRef.current === 'scheduled')) {
      ...
      const timer = setInterval(() => {
        val -= 1;
        if (val > 0) { setLaunchOverlay(val); playCountdownSound(); } 
        else if (val === 0) { setLaunchOverlay('GO!'); playBidSound(); } 
        else { clearInterval(timer); setLaunchOverlay(null); }
      }, 900);
    }
    lastPhaseRef.current = roomState.phase;
  }, [roomState?.phase]);

  useEffect(() => { if (chatRef.current) chatRef.current.scrollTop = chatRef.current.scrollHeight; }, [roomState?.chat]);
  ...
  useEffect(() => {
    if (roomState?.phase === 'bidding' && timeLeft > 0 && timeLeft <= 5 && timeLeft !== lastTimeRef.current) playCountdownSound();
    lastTimeRef.current = timeLeft;
  }, [timeLeft, roomState?.phase]);

  useEffect(() => { if (roomState?.phase === 'sold') playSoldSound(); }, [roomState?.phase]);
    ...
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, padding: 24, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Rajdhani', sans-serif", color: 'var(--t2)', fontSize: 15, maxWidth: 420, lineHeight: 1.6 }}>
          Room <strong style={{ color: 'var(--t1)' }}>{roomId}</strong> doesn't exist or has expired. Ask the host for a current invite code.
        </div>
  ...
  const currentBidAmt = roomState.currentBid || 0;
  const bidIncrement = currentBidAmt >= 50000000 ? 5000000 : 2000000;

  // BUDGET PROTECTION MATH
    ...
    <header className="global-sticky-header">
      ...
      <nav className="nav-container">
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <span className="hide-mobile" style={{ background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', border: '1px solid rgba(255,255,255,0.1)', padding: '4px 10px', fontSize: 11, borderRadius: 6, fontFamily: "'Rajdhani', sans-serif", fontWeight: 700 }}>Active Room</span>
        </div>
                      ...
                      <div key={p.id} className="card hover-lift" style={{ padding: 12, border: `1px solid ${p.color}${isUserTeam ? '' : '33'}`, background: isUserTeam ? `${p.color}08` : 'var(--bg3)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div style={{ overflow: 'hidden' }}>
                            ...
                            <div style={{ fontSize: 10, color: 'var(--t3)', marginTop: 2 }}>{getParticipantStatus(p.ownerId)}</div>
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div className="card hover-lift" style={{ padding: 16 }}>
                ...
                <div className="stats-grid">
                  ...
                  <div style={{ background: 'var(--bg3)', padding: 8, borderRadius: 8, textAlign: 'center' }}>
                    ...
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 15, color: 'var(--g)', marginTop: 2 }}>{formatCurrency(roomState.budget)}</div>
                  </div>
                  <div style={{ background: 'var(--bg3)', padding: 8, borderRadius: 8, textAlign: 'center' }}>
                ...
                {isScheduled ? (
                  <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ background: 'var(--bg3)', border: '1px solid var(--bd2)', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 11, fontWeight: 700, color: 'var(--am)', letterSpacing: 1.5, textTransform: 'uppercase' }}>📅 Scheduled Auction</div>
                      {allTeamsJoined ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                          <div style={{ fontSize: 12, color: 'var(--t2)', fontWeight: 600 }}>Waiting for teams ({joinedTeams}/{totalTeams})</div>
                          <div style={{ fontSize: 10, color: 'var(--t3)' }}>Starts once everyone joins & scheduled time arrives</div>
                        </div>
                      )}
                    </div>
                    {isHost && (
  ...
  const availLeft = (p: { id: string; budget: number; spent: number }) => {
    const pending = (roomState.currentBidder === p.id && (roomState.phase === 'bidding' || roomState.phase === 'sold')) ? roomState.currentBid : 0;
    return (p.budget || 0) - (p.spent || 0) - pending;
  };

  const isBidDisabled = roomState.phase !== 'bidding' || 
                        roomState.currentBidder === teamId || 
                        hasPassed || 
                        submitting || 
                        spotsLeft <= 0 || 
                        (me && (me.spent || 0) + currentBidAmt + bidIncrement > (me.budget || 0)) ||
                        (currentBidAmt + bidIncrement > maxBidAllowed && spotsLeft > 0);
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflow: 'hidden' }}>
      ...
      <div className="header-bar-main">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 12, color: 'var(--t3)' }}><span className="hide-mobile">{roomState.playerIdx + 1}/{safePlayers.length} players</span></div>
          ...
          <div className="auction-sidebar-left">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {safeParticipants.map((p) => {
                ...
                return (
                  <div key={p.id} className="card hover-lift" style={{ padding: '10px 12px', border: `1px solid ${isBidder ? p.color : p.color + '44'}`, background: isBidder ? p.color + '11' : p.id === teamId ? 'rgba(0, 220, 114, 0.04)' : 'var(--bg2)', boxShadow: isBidder ? `0 0 16px ${p.color}44, inset 0 0 8px ${p.color}22` : 'none', transform: isBidder ? 'scale(1.02)' : 'none', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <Avatar name={p.name} size={28} color={p.color} photo={p.photo} />
                      <span style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: 13, color: p.color, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name} {p.ownerId === userId ? '(You)' : ''}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--t2)', alignItems: 'center' }}>
                      <span><strong style={{ color: isBidder ? p.color : 'var(--t1)', fontFamily: "'Bebas Neue', sans-serif", fontSize: 15, letterSpacing: 1 }}>{formatCurrency(availLeft(p))}</strong> left</span>
            ...
            {isHost && (
              <div style={{ background: 'var(--bg3)', border: '1px solid var(--bd2)', borderRadius: 8, padding: '8px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {roomState.phase === 'paused' ? ( <button className="btn bp bsm" onClick={() => performAction('RESUME')} style={{ padding: '4px 12px' }}>▶️ Resume Auction</button> ) : ( <button className="btn bs bsm" onClick={() => performAction('PAUSE')} disabled={isEnd} style={{ padding: '4px 12px' }}>⏸️ Pause</button> )}
              ...
              <div key={`end${roomState.playerIdx}`} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, minHeight: 300 }}>
                ...
                {roomState.phase === 'sold' && (
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 46, color: 'var(--g)', letterSpacing: 3 }}>{formatCurrency(roomState.currentBid)}</div>
                    {win && (
              ...
              <div key={`bid${roomState.playerIdx}`} style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
                {pl && (
                  <div className="card hover-lift player-card-mobile" style={{ border: `2px solid ${(ROLE_COLORS[pl.role] || '#888')}33`, padding: 18, display: 'flex', gap: 14, alignItems: 'center', animation: 'slideIn .4s cubic-bezier(0.4, 0, 0.2, 1)' }}>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: 'var(--t2)', letterSpacing: 2 }}>{formatCurrency(pl.base)}</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div className={`card ${timeLeft <= 5 ? 'anim-urgent' : 'hover-lift'}`} style={{ textAlign: 'center', padding: 14 }}>
                    ...
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(40px, 6vw, 62px)', color: tc, letterSpacing: 2, lineHeight: 1, transition: 'color .3s' }}>{String(timeLeft).padStart(2, '0')}</div>
                  </div>
                  <div className="card hover-lift" style={{ textAlign: 'center', padding: 14 }}>
                    ...
                    <div key={roomState.currentBid} className="anim-pop" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(40px, 6vw, 62px)', color: 'var(--g)', letterSpacing: 2, lineHeight: 1 }}>{formatCurrency(roomState.currentBid)}</div>
                    {win ? <div style={{ fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: 13, color: win.color, marginTop: 4, letterSpacing: .5 }}>Now {win.name}'s turn</div> : <div style={{ color: 'var(--t3)', fontSize: 12, marginTop: 2 }}>No bids yet</div>}
                  </div>
                </div>
                ...
                <div className="card" style={{ padding: 13 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 9 }}>
                    ...
                    {hasPassed && <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', color: 'var(--re)', background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 6, padding: '3px 8px' }}>🙅 You passed</span>}
                  </div>
                  
                  <div style={{ display: 'flex', gap: 8, marginBottom: 9 }}>
                    <button className="btn bp"
                      onClick={() => performAction('BID', { bidder: teamId, amount: bidIncrement })}
                      disabled={isBidDisabled}
                      style={{ width: '100%', padding: '18px 6px', fontSize: 24, fontFamily: "'Bebas Neue', sans-serif", letterSpacing: 1.5, borderWidth: 2, opacity: isBidDisabled ? 0.45 : 1 }}>
                  ...
                  {isHost && (
                    <button className="btn bs" onClick={() => { const msg = roomState.currentBidder ? 'Skip this player? The current standing bid will be discarded.' : 'Skip this player? With no bids, they will be marked unsold.'; if (confirm(msg)) performAction('SKIP'); }} disabled={!canSkip || submitting} style={{ width: '100%', marginTop: 9, padding: '12px 6px', fontSize: 14, fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, letterSpacing: 1, borderWidth: 2, borderColor: 'var(--am)', color: 'var(--am)', opacity: canSkip ? 1 : 0.45, cursor: canSkip ? 'pointer' : 'not-allowed' }}>
                ...
                : safeBidHistory.map((b, i) => {
                  ...
                  return (
                    <div key={b.id || i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '5px 0', borderBottom: '1px solid var(--bd)', opacity: Math.max(.2, 1 - i * .08), animation: i === 0 ? 'slideIn .4s cubic-bezier(0.4, 0, 0.2, 1)' : 'none' }}>
                      ...
                      <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 13, color: 'var(--g)', letterSpacing: .5 }}>{formatCurrency(b.amount)}</span>
                    </div>
                  );
                })
              }
      ...
      {tab === 'myteam' && me && (
        <div style={{ padding: 22, maxWidth: 900, margin: '0 auto', width: '100%', overflowY: 'auto', maxHeight: 'calc(100vh - 54px)' }}>
          ...
          <div className="stats-grid-4">
            {[
              { l: 'Budget Left', v: formatCurrency((me.budget || 0) - (me.spent || 0)), c: 'var(--g)' },
              { l: 'Spent', v: formatCurrency(me.spent || 0), c: 'var(--or)' },
              { l: 'Players', v: `${(me.squad || []).length}/${roomState.squadSize}`, c: 'var(--bl)' },
              { l: 'Avg Price', v: (me.squad || []).length ? formatCurrency(Math.round((me.spent || 0) / (me.squad || []).length)) : '—', c: 'var(--pu)' },
            ].map((s) => (
              ...
              {(me.squad || []).map((p: any, i: number) => (
                <div key={String(p.id)} className="card hover-lift" style={{ padding: 13, border: `1px solid ${(ROLE_COLORS[p.role] || '#888')}33`, animation: `fadeUp 0.4s ease forwards`, animationDelay: `${i * 0.05}s`, opacity: 0 }}>
                  <div style={{ display: 'flex', gap: 5, marginBottom: 6 }}>
                    <RBadge role={p.role} />
                    {p.tier && p.tier.trim() !== '' && <span style={{ fontSize: 9, background: 'rgba(255,255,255,0.06)', border: '1px solid var(--bd)', padding: '2px 6px', borderRadius: 4, fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, color: 'var(--t2)', textTransform: 'uppercase' }}>T{p.tier.replace(/\D/g, '') || p.tier}</span>}
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 22, color: 'var(--g)', letterSpacing: 1 }}>{formatCurrency(p.soldPrice)}</span>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div key={t.id} style={{ border: `1px solid ${t.color}33`, borderRadius: 12, padding: 18, background: 'var(--bg2)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: 'var(--t3)', fontSize: 12 }}>{(t.squad || []).length} players · {formatCurrency(t.spent)} spent of {formatCurrency(t.budget)}</div>
                  </div>
                </div>
                {(t.squad || []).length === 0
                    ...
                    {(t.squad || []).map((p: any) => (
                      <div key={String(p.id)} className="card" style={{ padding: '12px 14px', border: `1px solid ${t.color}22`, background: 'var(--bg1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          ...
                          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 20, color: 'var(--g)' }}>{formatCurrency(p.soldPrice)}</div>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {tab === 'analytics' && (
        <div style={{ padding: 22, maxWidth: 1000, margin: '0 auto', width: '100%', overflowY: 'auto', maxHeight: 'calc(100vh - 54px)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
            ...
            <button className="btn bp" onClick={handleDownloadFullReport}>📥 Download Full Report</button>
          </div>
          <div className="stats-grid-4">
            {[
              { l: 'Players Sold', v: safeSoldLog.length, c: 'var(--g)' },
              { l: 'Unsold', v: safeUnsoldLog.length, c: 'var(--re)' },
              { l: 'Total Spent', v: formatCurrency(safeParticipants.reduce((a, b) => a + (b.spent || 0), 0)), c: 'var(--or)' },
              { l: 'Avg Sale', v: safeSoldLog.length ? formatCurrency(Math.round(safeSoldLog.reduce((a, b) => a + b.price, 0) / safeSoldLog.length)) : '—', c: 'var(--bl)' },
            ].map((s) => (
              <div key={s.l} className="card" style={{ textAlign: 'center', padding: 13 }}>
                <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 9, color: 'var(--t3)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 3 }}>{s.l}</div>
                <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 28, color: s.c, letterSpacing: 2 }}>{s.v}</div>
      ...
      {launchOverlay !== null && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(5, 7, 14, 0.88)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', zIndex: 9999, color: '#fff' }}>
          ...
          <div style={{ fontSize: '15vw', letterSpacing: 4, color: launchOverlay === 'GO!' ? 'var(--am)' : 'var(--g)', fontFamily: "'Bebas Neue', sans-serif" }}>{launchOverlay}</div>
        </div>
      )}
    </div>
  );
